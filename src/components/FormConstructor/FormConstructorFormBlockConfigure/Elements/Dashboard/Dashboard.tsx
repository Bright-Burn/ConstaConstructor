...
import {
    barChart1Data,
    barChart2Data, barChart3Data,
    pieChart1Data,
    pieChart2Data, pieChart3Data, pieChart4Data,
    selectIdleStepData,
    selectIdleTypeData
} from "./mocks";
...
export const Dashboard: FC<IPlaceholderFormElement> = ({element}) => {
    ...
    const PieWithLabel = ({ data, title, withStatistic, color, css }: ISmallPieProps) => (
        <div className={style.pie}>
                ...
                statistic={{
                    ...
                    content: withStatistic ? {
                        customHtml: (v, v2, v3  , v4) => {
                            ...
                            return <Text size="xl">{v3?.value || (Math.round(sum * 100) / 100).toLocaleString()}</Text>;
                        }
                    }: false,
                }}
            />
            {title && <div style={{
                height: '42px',
            }}><Text size="xs" align="center">{title}</Text></div>}
        </div>
    );

    const Cell = ({children, grow, direction, css}: {direction: 'row' | 'column', grow?: string, css?: React.CSSProperties, children: React.ReactNode}) => (
        <div style={{
            display: 'flex',
            ...
            <div style={{
                ...
                <Layout direction="column" className={style.root}>
                        ...
                        <Cell direction="column">
                            <div className={style.pane}>
                                <div style={{marginBottom: '10px'}}>
                                    <Text view="primary" weight="bold" size="2xl">КПЭ</Text>
                                </div>
                                <Layout direction="column" flex={1}>
                                    <Cell direction="row" grow="1">
                                        <Cell direction="column" grow="6">
                                            <PieWithLabel data={pieChart1Data} withStatistic={true} title="Коэф. достижения целевого"></PieWithLabel>
                                        </Cell>
                                        <Cell direction="column" grow="6">
                                            <PieWithLabel data={pieChart2Data} withStatistic={true} title="Qn т./сут."></PieWithLabel>
                                        </Cell>
                            ...
                            <div className={style.pane}>
                                <Cell direction="column">
                                    <Cell direction="row" grow="0" css={{alignItems: 'center'}}>
                                        <div style={{marginBottom: '10px'}}>
                                            <Text view="primary" weight="bold" size="2xl">Простои</Text>
                                        </div>
                                        <div style={{
                                            width: '180px',
                                            marginLeft: '20px',
                                    ...
                                    <Cell direction="row" grow="1">
                                        <Cell direction="row" grow="1">
                                            <PieWithLabel data={pieChart3Data} withStatistic={false} color={['#56b9f2', '#33b4ff80', '#eb5757']}></PieWithLabel>
                                        </Cell>
                    ...
                    <Cell direction="row">
                        <div className={style.pane}>
                            <div style={{marginBottom: '10px'}}>
                                <Text view="primary" weight="bold" size="2xl">Затраты</Text>
                            </div>
                            <Cell direction="row">
                                <Cell direction="column" grow="1">
                                    <PieWithLabel data={pieChart4Data} title="" withStatistic={false} css={{
                                        width: '125px',
                                        height: '125px',
                                    }}></PieWithLabel>
                                    <Stats
